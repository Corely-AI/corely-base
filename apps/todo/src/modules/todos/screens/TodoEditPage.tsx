...
import { type CreateTodoInput, type TodoPriority, type TodoStatus, type UpdateTodoInput } from "@corely/contracts";
...
export const TodoEditPage: React.FC = () => {
  ...
  const mutation = useMutation({
    ...
    onSuccess: async (savedTodo) => {
      ...
      navigate(isNew ? `/todos/${savedTodo.id}` : `/todos/${id}`);
    },
    onError: (error: any) => {
      toast({
        title: "Action failed",
      ...
      <Card className="rounded-[2rem] border-border/70 bg-background/85 shadow-sm">
        <CardHeader className="space-y-2">
          ...
          <p className="text-sm text-muted-foreground">
            Keep it small and specific. The API accepts the same payload as the main web app.
          </p>
        </CardHeader>
